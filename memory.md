...
- **当前工作分支**：`main-v0.5.0-audit`（合并提交 `03cfb52`）；原 `main` 由另一工作树占用，仍指向合并前提交，需在主工作树中快进到 `03cfb52`；
- **跟踪上游基线**：已合入 `upstream/main`（Tag: `v0.5.0`，提交 `a3b4df1`）；
- **当前产物版本**：`manifest.json` 版本号 `0.5.0`，缓存键 `20260924.360-09f2d0eacb2e3aed`（bundle SHA-256 前 16 位）；
- **v0.5.0 上游能力**：千事时间线与候选召回、时间事项批量永久删除、白鸟存储暖快照与失败退避、召回候选总预算整合；本地清洗器和召回失效闭环均已保留；
- **本次验证**：针对性测试 376/376 通过；全量测试 1141/1143 通过。剩余 2 项为缺少外部 `ST-SevenDaysCal` 夹具及已知负载敏感的 `MESSAGE_DELETED` 时序偶发，单独重跑后通过；
- **最近提交记录**：
