...
- **治本双模块闭环架构设计**：
  1. **密封点活版本重对齐 (`commitPromptIfCurrent`)**：
     选材 LLM 耗费 20 多秒返回后，在持久化回执前重新读取活的档案头 `store.readRoot()`；当检测到并发推进（无论是摘要归档还是地基对齐）且已选材料依然兼容时，直接将收据内部盖章的 `headCheckpointId`、`rootRevision` 和签名 `receiptFingerprint` 自动重对齐为当前最新的档案版本；使收据在写入数据库时天然与当前世界状态一致，第 1 次重新生成即可秒级复用（95ms）；
  2. **正文见证楼层向前截断 (`captureCoreBodyWitness`)**：
     传入触发用户楼 `userMessage`，严格从该用户楼之前逆向采集见证楼层，彻底杜绝宿主尾部未确认 AI 楼层残留引起的见证指纹漂移；
  3. **严谨的 OCC 一致性保留**：
